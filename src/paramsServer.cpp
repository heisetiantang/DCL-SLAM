...
//   Eigen::Quaterniond rotation_eigen(rotation.toQuaternion().w(), rotation.toQuaternion().x(),
// 									rotation.toQuaternion().y(), rotation.toQuaternion().z());
//   Eigen::Affine3f transform = Eigen::Translation3f(translation_eigen) * rotation_eigen;

  Eigen::Affine3f trans_cur = pcl::getTransformation(pose->x, pose->y, pose->z, pose->roll, pose->pitch, pose->yaw);
//   trans_cur = transform * trans_cur;
