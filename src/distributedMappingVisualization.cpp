...
bool flg_map_merge = false;
std::chrono::system_clock::time_point next_time_save = std::chrono::system_clock::now();
std::chrono::system_clock::time_point next_time_merge = std::chrono::system_clock::now();
...
void distributedMapping::globalMapThread()
{
  ...
  while (ros::ok())
  {
    ...
    // {
    //   if (std::chrono::system_clock::now() >= next_time_merge)
    //   {
    //     next_time_merge = std::chrono::system_clock::now() + std::chrono::seconds(20);
    //     flg_map_merge = true;
    //   }
